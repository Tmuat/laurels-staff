...
def retrieve_board_call(board_instance, instance):
...
@otp_required
@login_required
def retrieve_board(request, board_id):
    """
    Ajax URL for retrieving a board to the signmaster api.
    """
    data = dict()
    ...
    parsed_xml = parse_xml(get_next_active_date())
    activate_date = datetime.datetime.strptime(
        parsed_xml.find("activedate").text, "%Y-%m-%d"
    ).date()

    if request.method == "POST":
        form = RetrieveBoardForm(request.POST)
        if form.is_valid():
            data["form_is_valid"] = True
            # instance = form.save(commit=False)
            # new_board_request = new_board(board_instance, instance)
    ...
    data["html_modal"] = render_to_string(
        "boards/includes/retrieve_board.html",
        context,
        request=request,
    )
